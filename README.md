...
**Price.** Levels are multiples of powers of three — 1, 3, 9, 27, 81, 243,
729, 2187, 6561, 19683 — drawn around current price. Nothing is fitted,
optimised or inferred from price action: a level either is a multiple of a
power of three or it is not.

The ladder starts at **3⁰ = 1**, one checkbox like the other nine, and the one
that is **off by default**. At scale 1 it is every whole number, so on gold it
draws a line per dollar and every 3 cell on the chart gets its two interior
lines — the finest subdivision the model has, and more than a chart wants
standing on it unasked. Tick it when the question is where inside a 3 range
price is sitting.
...
tick the box and it appears when you drop to M1, and the Experts log says so on
load rather than leaving a ticked box silently drawing nothing.
...
- One checkbox per PO3 number from 3⁰ = 1 up to 3⁹ = 19683, each with its own colour, the 1 grid off by default and drawn very faint on M1 only as the subdivision of the 3 grid
...
| Show 1 | `false` | 3⁰, first in the list, **off by default** and **M1 only**. Every whole number at scale 1 — a line per dollar on gold, subdividing each 3 cell into thirds. Drawn in a near-black so it reads as texture, not as a level. Ticked on a higher timeframe it draws nothing and says so in the log |
...
| ... and keep the server time beside it | `false` | Appends the broker's time of day in brackets, widening the block |
| Write the times as AM / PM, not 24-hour | `true` | The timetable only. The count and segment blocks still print period opens in 24-hour |
...
```
Week   9-33 from 2025.09.15  UTC+10    Day    ahead from  7:00 AM  UTC+10
  D1    9 ~Thu 25/09  7:00 AM due      > H1    9 ~Mon 15/09  3:00 PM due
       17 ~Tue 07/10  7:00 AM due      >      17 ~Mon 15/09 11:00 PM due
       26 ~Mon 20/10  7:00 AM due
       33 ~Wed 29/10  7:00 AM due        M30   9 ~Mon 15/09 11:00 AM due
                                              17 ~Mon 15/09  3:00 PM due
  H4    9  Tue 16/09  3:00 PM done            26 ~Mon 15/09  7:30 PM due
       17  Wed 17/09 11:00 PM NOW             33 ~Mon 15/09 11:00 PM due
       26 ~Fri 19/09 11:00 AM due
       33 ~Mon 22/09  3:00 PM due
```
...
**The times are on the 12-hour clock**, AM and PM, which is the default. The
timetable is the block whose times get written into a diary, so it is the one
that reads the way a diary does. Midnight is `12:00 AM` and noon `12:00 PM` —
the one place the 12-hour clock catches people out. Untick *Write the times as
AM / PM* for the 24-hour form. The count and segment blocks are unaffected
either way: they print period opens, not appointments.
...
Tick *and keep the server time beside it* to get both: `~Thu 25/09  7:00 AM
(12:00 AM)`, the bracketed one being the broker's, on the same clock. Time of day only — the date is
...
| *Levels each side of price* | 1–100 | **1–25**, so nine grids fit in 450 lines; the 1 grid's triple window is MT5 only |
